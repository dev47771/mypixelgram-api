...
export const DESCRIPT_TEXT_GITHUB_CALLBACK =
  'Receives the callback from GitHub after successful authorization. The endpoint extracts user data (GitHub ID, username, email, avatar, tokens) and completes login or registration in the system. On success, sets secure HttpOnly cookies and redirects to a success URL. On failure, redirects to an error URL.';
...
export const DESCRIPT_TEXT_GOOGLE_CALLBACK =
  'Handles the callback from Google after successful user authorization. Extracts user data (Google ID, email, username, avatar, tokens) and completes registration or login in the system. On success, sets an HttpOnly refresh token cookie and redirects to the profile page. On error, redirects to the error page.';
